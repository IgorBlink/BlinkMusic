...
exports.trackService = exports.TrackService = void 0;
const logger_1 = require("../utils/logger");
const Track_1 = __importDefault(require("../models/Track"));
const lastFmService_1 = require("./lastFmService");
const audioDbService_1 = require("./audioDbService");
const mongoose_1 = __importDefault(require("mongoose"));
const axios_1 = __importDefault(require("axios"));
const audioDbService_2 = require("./audioDbService");
class TrackService {
    constructor() {
        this.audioDbService = new audioDbService_2.AudioDbService();
        this.lastFmService = lastFmService_1.lastFmService;
    }
    /**
     * Получить все треки с пагинацией (фактически возвращает популярные треки из Last.fm)
     */
    async getAllTracks(page = 1, limit = 10, sort = '-createdAt') {
        try {
            logger_1.logger.info(`Getting all tracks with pagination: page=${page}, limit=${limit}, sort=${sort}`);
            ...
            const enrichedTracks = await Promise.all(lastFmTracks.map(async (track) => {
            ...
            logger_1.logger.error(`Error getting all tracks: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async getTrackById(id) {
        try {
            ...
            if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
                logger_1.logger.warn(`Некорректный ID трека: ${id}, пробуем искать через Last.fm`);
                // Пытаемся интерпретировать id как "artist-title"
                if (id.includes('-')) {
                    const [artist, title] = id.split('-').map(part => part.trim());
                    if (artist && title) {
                        logger_1.logger.info(`Искаем трек через Last.fm: ${artist} - ${title}`);
            ...
            logger_1.logger.info(`Getting track by ID: ${id}`);
            ...
            if (!track.coverUrl || track.coverUrl.includes('2a96cbd8b46e442fc41c2b86b821562f')) {
                logger_1.logger.info(`Обновляем обложку для трека ${track.title}`);
            ...
            logger_1.logger.error(`Error getting track by ID: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async searchTracks(query, limit = 10) {
        try {
            logger_1.logger.info(`Searching tracks with query: "${query}", limit: ${limit}`);
            // Получаем треки из Last.fm
            logger_1.logger.info(`Searching for "${query}" in Last.fm`);
            ...
            logger_1.logger.error(`Error searching tracks: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async getTracksByGenre(genre, limit = 10) {
        try {
            logger_1.logger.info(`Getting tracks by genre: "${genre}", limit: ${limit}`);
            // Получаем треки из Last.fm
            const lastFmTracks = await this.lastFmService.getTracksByGenre(genre, limit);
            logger_1.logger.info(`Found ${lastFmTracks.length} tracks of genre "${genre}" from Last.fm`);
            // Обогащаем каждый трек обложкой из iTunes
            const enrichedTracks = await Promise.all(lastFmTracks.map(async (track) => {
                // Получаем обложку из iTunes
                const itunesCover = await this.getITunesCover(track.artist, track.title);
            ...
            logger_1.logger.error(`Error getting tracks by genre: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async getRecommendedTracks(limit = 10) {
        try {
            logger_1.logger.info(`Getting recommended tracks, limit: ${limit}`);
            // Получаем популярные треки из Last.fm
            const lastFmTracks = await this.lastFmService.getTopTracks(limit);
            logger_1.logger.info(`Found ${lastFmTracks.length} popular tracks from Last.fm`);
            // Обогащаем каждый трек обложкой из iTunes
            const enrichedTracks = await Promise.all(lastFmTracks.map(async (track) => {
                // Получаем обложку из iTunes
                const itunesCover = await this.getITunesCover(track.artist, track.title);
            ...
            logger_1.logger.error(`Error getting recommended tracks: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async addToFavorites(userId, trackId) {
        try {
            ...
            logger_1.logger.error(`Error adding track to favorites: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async removeFromFavorites(userId, trackId) {
        try {
            ...
            logger_1.logger.error(`Error removing track from favorites: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     */
    async enrichTrackWithMetadata(track) {
        var _a;
        try {
            logger_1.logger.info(`Обогащаем метаданными трек "${track.title}" от "${track.artist}"`);
            ...
            if (!track.audioUrl.includes('youtube.com')) {
                logger_1.logger.info(`Получаем YouTube-ссылку для трека "${track.title}" от "${track.artist}"`);
                ...
                try {
                    ...
                    if (youtubeUrl) {
                        logger_1.logger.info(`Обновляем аудио-ссылку на YouTube URL для "${track.title}"`);
                        track.audioUrl = youtubeUrl;
                        // Сохраняем изменения в базу данных
                        await Track_1.default.findByIdAndUpdate(track._id, { audioUrl: youtubeUrl });
                        ...
                        if (!track.duration || track.duration === 0) {
                            try {
                                ...
                                if (videoDuration > 0) {
                                    logger_1.logger.info(`Обновляем длительность трека "${track.title}" на ${videoDuration}ms из YouTube`);
                                    track.duration = videoDuration;
                                    // Сохраняем длительность в базу данных
                                    await Track_1.default.findByIdAndUpdate(track._id, { duration: videoDuration });
                                }
                            }
                            catch (durationError) {
                    ...
                    logger_1.logger.error(`Ошибка при получении YouTube-ссылки: ${error}`);
                }
            }
            ...
            if (track.enriched && Object.keys(track.enriched).length > 0 &&
                ...
                logger_1.logger.info(`Трек "${track.title}" уже обогащен метаданными`);
                return track;
            }
            ...
            if (audioDbData) {
                logger_1.logger.info(`Получены метаданные AudioDB для "${track.title}" от "${track.artist}"`);
                ...
                if (audioDbData.albumCover &&
                    ...
                    const isDefaultLastFmCover = (_a = track.coverUrl) === null || _a === void 0 ? void 0 : _a.includes('2a96cbd8b46e442fc41c2b86b821562f');
                    if (!track.coverUrl || isDefaultLastFmCover) {
                        logger_1.logger.info(`Обновляем обложку для "${track.title}" на обложку из AudioDB`);
                        track.coverUrl = audioDbData.albumCover;
                    }
                }
                // Применяем изменения к базе данных
                const updatedTrack = await Track_1.default.findByIdAndUpdate(track._id, {
                    enriched: track.enriched,
                    albumName: track.albumName,
                    coverUrl: track.coverUrl,
                    audioUrl: track.audioUrl
                }, { new: true });
                logger_1.logger.info(`Успешно обогащен трек "${track.title}" метаданными и сохранен в БД`);
                // Возвращаем обновленный трек
                return updatedTrack ? updatedTrack : track;
            }
            else {
                logger_1.logger.info(`Не найдено дополнительных метаданных для "${track.title}" от "${track.artist}"`);
            }
            return track;
        }
        catch (error) {
            logger_1.logger.error(`Ошибка при обогащении трека метаданными: ${error}`);
     ...
     */
    async enhanceTrackCover(track) {
        var _a, _b;
        try {
            // Проверяем, требуется ли обновление обложки
            if (!track.coverUrl) {
                logger_1.logger.info(`Трек "${track.title}" не имеет обложки, пропускаем улучшение`);
                return;
            }
            ...
            if (isDefaultLastFmCover) {
                logger_1.logger.info(`У трека "${track.title}" дефолтная обложка Last.fm, пытаемся найти лучше`);
            }
            ...
            if (track.coverUrl.includes('itunes.apple.com')) {
                try {
                    logger_1.logger.info(`Получаем обложку из iTunes для "${track.title}"`);
                    const response = await axios_1.default.get(track.coverUrl);
                    if (((_a = response.data) === null || _a === void 0 ? void 0 : _a.results) && response.data.results.length > 0) {
                        ...
                        if (artworkUrl) {
                            ...
                            logger_1.logger.info(`Обновляем обложку для "${track.title}" из iTunes`);
                            track.coverUrl = highResArtwork;
                            await track.save();
                        }
                    }
                    else {
                        logger_1.logger.info(`iTunes не вернул обложку для "${track.title}"`);
                    }
                }
                catch (error) {
                    logger_1.logger.error(`Ошибка при получении обложки из iTunes: ${error}`);
                }
            }
            // Обрабатываем запрос к Deezer API
            else if (track.coverUrl.includes('api.deezer.com')) {
                try {
                    logger_1.logger.info(`Получаем обложку из Deezer для "${track.title}"`);
                    const response = await axios_1.default.get(track.coverUrl);
                    if (((_b = response.data) === null || _b === void 0 ? void 0 : _b.data) && response.data.data.length > 0) {
                        const album = response.data.data[0].album;
                        if (album && album.cover_big) {
                            logger_1.logger.info(`Обновляем обложку для "${track.title}" из Deezer`);
                            track.coverUrl = album.cover_big;
                            await track.save();
                        }
                    }
                    else {
                        logger_1.logger.info(`Deezer не вернул обложку для "${track.title}"`);
                    }
                }
                catch (error) {
                    logger_1.logger.error(`Ошибка при получении обложки из Deezer: ${error}`);
                }
            }
            ...
            if (isDefaultLastFmCover || !track.coverUrl) {
                try {
                    ...
                    const audioDbData = await audioDbService_1.audioDbService.getTrackInfo(track.artist, track.title);
                    if ((audioDbData === null || audioDbData === void 0 ? void 0 : audioDbData.albumCover) &&
                        audioDbData.albumCover !== 'none' &&
                        !audioDbData.albumCover.includes('lastfm') &&
                        !audioDbData.albumCover.includes('2a96cbd8b46e442fc41c2b86b821562f')) {
                        logger_1.logger.info(`Обновляем обложку для "${track.title}" из AudioDB`);
                        track.coverUrl = audioDbData.albumCover;
                        await track.save();
                    }
                }
                catch (error) {
                    logger_1.logger.error(`Ошибка при получении обложки из AudioDB: ${error}`);
                }
            }
        }
        catch (error) {
            logger_1.logger.error(`Ошибка при улучшении обложки трека: ${error}`);
        }
    }
    /**
     ...
     */
    async forceUpdateTrackCover(track) {
        var _a;
        try {
            logger_1.logger.info(`Trying iTunes API for "${track.title}" by "${track.artist}"`);
            ...
            if (((_a = itunesResponse.data) === null || _a === void 0 ? void 0 : _a.results) && itunesResponse.data.results.length > 0) {
                ...
                if (artworkUrl) {
                    ...
                    if (track._id) {
                        logger_1.logger.info(`Found better cover for "${track.title}" by "${track.artist}" from iTunes`);
                        // Обновляем обложку трека в базе данных
                        await Track_1.default.findByIdAndUpdate(track._id, {
            ...
            logger_1.logger.info(`No cover found in iTunes for "${track.title}" by "${track.artist}"`);
            return track;
        }
        catch (error) {
            logger_1.logger.error(`Error updating track cover: ${error}`);
     ...
     */
    async getITunesCover(artist, title) {
        var _a;
        try {
            logger_1.logger.info(`Getting iTunes cover for "${title}" by "${artist}"`);
            ...
            logger_1.logger.info(`No cover found in iTunes for "${title}" by "${artist}"`);
            return null;
        }
        catch (error) {
            logger_1.logger.error(`Error getting iTunes cover: ${error}`);
            return null;
        }
    }
    /**
     ...
     */
    async updateExistingTracks() {
        try {
            logger_1.logger.info('Запуск принудительного обновления обложек для существующих треков');
            // Найти все треки с дефолтными обложками Last.fm
            const tracksWithDefaultCovers = await Track_1.default.find({
                coverUrl: { $regex: '2a96cbd8b46e442fc41c2b86b821562f' }
            }).limit(20); // Ограничиваем количество треков для одновременного обновления
            logger_1.logger.info(`Найдено ${tracksWithDefaultCovers.length} треков с дефолтными обложками`);
            ...
            logger_1.logger.info('Завершено обновление обложек');
        }
        catch (error) {
            logger_1.logger.error(`Ошибка при массовом обновлении обложек: ${error}`);
        }
    }
    /**
     ...
     */
    async createTrackFromLastFm(lastFmTrack) {
        logger_1.logger.info(`Creating new track from Last.fm: ${lastFmTrack.artist} - ${lastFmTrack.title}`);
        // Создаем базовый объект трека с данными из Last.fm
        const newTrack = new Track_1.default({
     ...
     */
    async forceUseITunesCoverForAllTracks(limit = 100) {
        try {
            logger_1.logger.info('Запуск принудительного обновления обложек из iTunes для всех треков');
            // Получаем треки с дефолтными обложками Last.fm или без обложек
            const tracksToUpdate = await Track_1.default.find({
                $or: [
                    { coverUrl: { $regex: '2a96cbd8b46e442fc41c2b86b821562f' } },
            ...
            logger_1.logger.info(`Найдено ${tracksToUpdate.length} треков для обновления обложек`);
            // Обновляем обложки для каждого найденного трека
            let updatedCount = 0;
            ...
            logger_1.logger.info(`Обновлено обложек: ${updatedCount} из ${tracksToUpdate.length}`);
        }
        catch (error) {
            logger_1.logger.error(`Ошибка при массовом обновлении обложек: ${error}`);
        }
