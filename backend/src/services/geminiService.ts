...
export class GeminiService {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      logger.warn('GEMINI_API_KEY не указан в .env файле. Аннотации не будут работать.');
    }
  }
  ...
  async generateAnnotation(
    ...
    try {
      ...
      if (response.data.candidates && 
        ...
        logger.error('Некорректный ответ от Gemini API:', response.data);
        return 'Не удалось получить аннотацию.';
      }
    } catch (error: any) {
      logger.error('Ошибка при получении аннотации от Gemini API:', error.message);
